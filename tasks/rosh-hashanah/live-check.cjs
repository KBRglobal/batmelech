...
async function run(){
 ...
 try{
  ...
  for(const locale of ['','/en','/fr']){
   ...
   const script=await page.locator('script[type="module"][src^="/site/assets/"]').getAttribute('src');assert.ok(fs.readFileSync(path.join(root,'site/index.html'),'utf8').includes(script));
  ...
  const adminScript=await page.locator('script[type="module"][src^="/app/assets/"]').getAttribute('src');assert.ok(fs.readFileSync(path.join(root,'web/dist/index.html'),'utf8').includes(adminScript));report.admin.script=adminScript;
