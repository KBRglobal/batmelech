...
async function run(){
 ...
 const server=await new Promise(resolve=>{const s=app.listen(4192,'127.0.0.1',()=>resolve(s))});const baseline=process.argv.includes('--baseline');const report={layouts:[],errors:[]};
 try{for(const engine of baseline?[chromium]:[chromium,webkit]){
  ...
  for(const locale of ['','/en','/fr'])for(const width of [320,390,430,768,1024]){
   ...
   if(width<1024){await p.locator('input[value="family"]').check();await guide.waitFor();assert.equal(await guide.locator('li').count(),8);assert.ok((await guide.innerText()).includes('$450'));await p.locator('input[value="four"]').check();assert.equal(await guide.locator('li').count(),7);assert.ok((await guide.innerText()).includes('$650'));await p.locator('input[value="couple"]').check();assert.ok((await guide.innerText()).includes('$350'));
    if(engine===chromium&&!locale&&width===390){await guide.scrollIntoViewIfNeeded();await p.screenshot({path:folder+'/mobile-clarity.png'});}
   }
   report.layouts.push({browser:engine.name(),locale:locale||'he',width});
  }
  await open('',390);await p.locator('input[value="couple"]').check();assert.equal(await p.locator('.festive-add-button').isDisabled(),true);await p.getByRole('spinbutton',{name:'קציצות דגים ברוטב מרוקאי',exact:true}).fill('1');await p.getByRole('spinbutton',{name:'קציצות בקר ברוטב אפונה וארטישוק',exact:true}).fill('1');await p.getByRole('spinbutton',{name:'אורז לבן קלאסי',exact:true}).fill('1');assert.equal(await p.getByTestId('festive-total').innerText(),'$350');await p.getByRole('spinbutton',{name:'דג ברוטב מרוקאי',exact:true}).fill('1');assert.equal(await p.getByTestId('festive-total').innerText(),'$380');await p.locator('.festive-add-button').click();await p.waitForURL('**/checkout');
 }finally{await browser.close()}}
 assert.deepEqual(report.errors,[]);fs.writeFileSync(folder+'/mobile-clarity.json',JSON.stringify(report,null,2));console.log(JSON.stringify(report));
