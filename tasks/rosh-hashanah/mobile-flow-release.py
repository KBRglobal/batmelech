...
review=root/'tasks/rosh-hashanah/evidence/mobile-flow-review.md';assert review.exists() and 'PASS' in review.read_text(), 'Review must pass'
...
run('git','diff','--cached','--check');run('git','commit','-m','Guide mobile package selection directly to required dish choices')
commit=run('git','rev-parse','HEAD');run('git','push','origin','main');report={'commit':commit,'pushed':True}
out=root/'tasks/rosh-hashanah/evidence/mobile-flow-release.json';out.write_text(json.dumps(report,indent=2))
for _ in range(35):
...
result=json.loads(run('node','tasks/rosh-hashanah/mobile-flow-live.cjs'));report['live']=result;out.write_text(json.dumps(report,indent=2));print(json.dumps(report))
