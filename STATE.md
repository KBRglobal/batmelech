...
**Telegram bot + group fully set up, notification code NOT written yet:**
- Bot `@batmelech_orders_bot`, group "הזמנות" with the bot + Lin both members (sidesteps
  the "bot can't DM first" limit entirely — group chat_id works immediately once the bot is
  added and one command is sent). Token + chat_id in `~/Documents/creds/batmelech-telegram.txt`,
  mirrored as Railway env vars `TELEGRAM_BOT_TOKEN` and `TELEGRAM_ORDERS_CHAT_ID` (`-5417329227`).
- **Next actual work, not done:** no code sends anything yet. To notify on a new order,
  POST to `https://api.telegram.org/bot<TOKEN>/sendMessage` with that chat_id — plain HTTPS,
  no SDK needed. Wire it into wherever an order actually lands
  (`server/site-order-route.js` for site orders; check whether admin-created/edited orders
  need the same hook).
