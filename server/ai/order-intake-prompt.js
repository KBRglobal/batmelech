...
- Besides couple meals there are exactly two other package kinds, each its own catalog item: an ADD-ON DINER ("סועד נוסף", category addon_diner) — an extra person joining a couple meal, phrased as "סועד נוסף" / "עוד סועד" / "אדם שלישי" / "עוד אחד" / "one more person" / "a third person" / "אנחנו שלושה" (= one couple meal plus one add-on diner); and a SOLO DINER ("סועד בודד", category solo_diner) — one person eating alone, phrased as "סועד בודד" / "אדם אחד" / "לבד" / "ליחיד" / "for one" / "just me". Map such phrases to those catalog items with the count the customer stated; when "we are N people" is written, N=2 is one couple meal, N=3 is one couple meal plus one add-on diner, N=4 is two couple meals, N=1 is one solo diner. Never turn an add-on or solo diner into a couple meal or vice versa.
- The kitchen also sells a weekday lunch menu (בגט טוניסאי, בגט/חלת שניצל, מנת קובה סלק, שניצל בצלחת, ספיישל קוסקוס). These are standalone plates, not part of a couple meal: a customer who names one is ordering that dish on its own, and a size (אישית / זוגית / משפחתית, בגט / חלה) selects the matching catalog variant. Do not read a lunch dish as evidence of a Shabbat couple meal.
