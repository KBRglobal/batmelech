...
- טיוטה שנשענת על פירוש שהשאלה לא קובעת (למשל "הלקוח הכי טוב" נענה לפי כסף בלי לשאול אם הכוונה לכסף או למספר הזמנות; "כמה שילם" נענה בסכום ההזמנה במקום במה שנגבה; שם שמתאים לכמה לקוחות) — לא נתמכת. במקרה כזה corrected היא שאלת הבהרה אחת קצרה עם האפשרויות, בלי נתונים.
- מה שמיי אומרת על עצמה — אילו כלים יש לה, איך היא עובדת, מה היא זוכרת — מגיע מההוראות שלה ולא נבדק כאן; רשימת הכלים שלה מצורפת לראיות.
...
אם לא: ok=false, problems=רשימת הטענות הבעייתיות (ציטוט + מה הראיות אומרות + איזה כלי/חיפוש היה נותן את התשובה, אם ברור), corrected=הטיוטה משוכתבת: אותו טון, אותה שפה, אותו אורך בערך — טענה לא נתמכת מוחלפת בעובדה הנתמכת מהראיות, ואם אין כזו — במשפט קצר שהיא בודקת ותחזור. לעולם לא ניסוחים כמו "אין לי הוכחה" או "לא ניתן לאמת" — ללין יש מערכת עם כל הנתונים; או שיש תשובה מבוססת או שבודקים שוב. לא להוסיף עובדות חדשות שאינן בראיות.`;
 ...
 */
async function verifyReply({ client, model, effort = 'low', evidence, userTurn, draft, toolNames = [], logger = console }) {
  const input = [
    { role: 'system', content: VERIFIER_INSTRUCTIONS },
    {
      ...
      content:
        ...
        `3. הכלים שיש למיי: ${toolNames.join(', ') || 'לא צוין'}\n\n` +
        `הטיוטה לבדיקה:\n${draft}`,
    },
  ];
  let response;
