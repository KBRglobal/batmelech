...
test('uses immutable output-token and reasoning limits on every provider request', async () => {
  const fake = fakeOpenAI({ status: 'completed', output: [], output_parsed: completeReview() });
  const reviewer = createOpenAIOrderIntake({
    client: fake.client,
    model: 'test-model',
  ...
  assert.equal(fake.calls[0].max_output_tokens, 6_000);
  assert.deepEqual(fake.calls[0].reasoning, { effort: 'medium' });
});
...
test('returns safe service errors for refusals, null output, and malformed output', async (t) => {
  ...
  await t.test('schema-valid output with invented catalog data', async () => {
  ...
  await t.test('fabricated source evidence', async () => {
    ...
    await assert.rejects(
      reviewer({ message: 'רק קוסקוס אחד', catalog }),
      (error) => error.code === SERVICE_ERROR_CODES.INVALID_PROVIDER_OUTPUT
    );
  });
  ...
  await t.test('quantity absent from grounded source evidence', async () => {
    ...
    await assert.rejects(
      reviewer({ message: 'שניצל בבקשה', catalog }),
      (error) => error.code === SERVICE_ERROR_CODES.INVALID_PROVIDER_OUTPUT
    );
  });
  ...
  for (const [field, inventedValue] of [
    ...
    await t.test(`invented draft ${field}`, async () => {
      ...
      await assert.rejects(
        reviewer({ message: 'הזמנה לבדיקה', catalog }),
        (error) => error.code === SERVICE_ERROR_CODES.INVALID_PROVIDER_OUTPUT
      );
    });
  }
  ...
  await t.test('paid extra omitted from paidExtras', async () => {
    const malformed = completeReview({
      draft: {
        items: [
          {
    ...
    await assert.rejects(
      reviewer({ message: 'עוד דג אחד', catalog }),
      (error) => error.code === SERVICE_ERROR_CODES.INVALID_PROVIDER_OUTPUT
    );
  });
